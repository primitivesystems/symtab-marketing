...
import { runThemeTransition } from "@workspace/ui/lib/theme-transition"
...
const Skiper26 = () => {
  ...
  return (
    <div className="relative flex h-full w-full flex-col items-center justify-center">
      ...
      <div className="grid content-start justify-items-center gap-6 py-20 text-center text-foreground">
        <span className="relative max-w-[12ch] text-xs leading-tight uppercase opacity-40 after:absolute after:start-1/2 after:top-full after:h-16 after:w-px after:bg-gradient-to-b after:from-background after:to-foreground after:content-['']">
...
const Options = ({
  ...
  return (
    <motion.div
      drag
      className="bg-muted2 absolute end-1/2 top-30 flex w-[245px] translate-x-1/2 flex-col gap-3 rounded-3xl border border-foreground/10 p-3 backdrop-blur-sm lg:end-4 lg:translate-x-0 rtl:-translate-x-1/2 rtl:lg:-translate-x-0"
    >
          ...
          <div className="flex flex-wrap items-center justify-end gap-1">
            <button
              ...
              className={cn(
                ...
                variant === "circle"
                  ? "opacity-100"
                  : "opacity-50 hover:bg-foreground/10 hover:opacity-100"
              )}
            >
              circle
            </Button>
        ...
        {variant === "gif" && gifType === "custom" && (
          <div className="mt-1 flex flex-col gap-1 py-1">
            ...
            <input
              ...
              className="w-full rounded-lg bg-transparent px-2 py-1 text-xs text-foreground placeholder:text-foreground/50 focus:outline-none"
            />
          </div>
        )}
      </div>
  ...
  const changeTheme = useCallback(
    (target?: "light" | "dark" | "system") => {
      ...
      return runThemeTransition(
        ...
        createAnimation(variant, start, blur, gifUrl).css
      )
    },
    [setTheme, variant, start, blur, gifUrl]
  )
  return {
    isDark: resolvedTheme === "dark",
    toggleTheme: () => changeTheme(),
    setIsDark: (dark: boolean) => changeTheme(dark ? "dark" : "light"),
    setCrazyLightTheme: () => changeTheme("light"),
    setCrazyDarkTheme: () => changeTheme("dark"),
    setCrazySystemTheme: () => changeTheme("system"),
  }
}
...
export const ThemeToggleButton = ({
  ...
  return (
    <button
      type="button"
      className={cn(
        "theme-toggle size-10 cursor-pointer rounded-full bg-black p-0 transition-transform duration-150 active:scale-[0.96]",
        className
      )}
      onClick={toggleTheme}
